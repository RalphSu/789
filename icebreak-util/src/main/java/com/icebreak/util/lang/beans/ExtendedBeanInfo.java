package com.icebreak.util.lang.beans;
...
class ExtendedBeanInfo implements BeanInfo {
	...
	private final SortedSet<PropertyDescriptor> propertyDescriptors = new TreeSet<PropertyDescriptor>(
		new PropertyDescriptorComparator());
	
	/**
	 * Wrap the given delegate {@link BeanInfo} instance and find any non-void
	 * returning setter methods, creating and adding a
	 * {@link PropertyDescriptor} for each.
	 * 
	 * <p>
	 * Note that the wrapped {@code BeanInfo} is modified by this process.
	 * 
	 * @see #getPropertyDescriptors()
	 * @throws IntrospectionException if any problems occur creating and adding
	 *             new {@code PropertyDescriptors}
	 */
	public ExtendedBeanInfo(BeanInfo delegate) throws IntrospectionException {
		...
		ALL_METHODS: for (MethodDescriptor md : delegate.getMethodDescriptors()) {
			...
			if (method.getName().startsWith("set") && method.getParameterTypes().length == 1) {
				...
				for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
					...
					if (writeMethod != null && writeMethod.getName().equals(method.getName())) {
						...
						this.addOrUpdatePropertyDescriptor(pd, propertyName, readMethod,
							writeMethod);
						continue ALL_METHODS;
					}
			...
			if (method.getName().startsWith("set") && method.getParameterTypes().length == 2
				...
				DELEGATE_PD: for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
					...
					if (indexedWriteMethod != null
						...
						// yes -> copy it, including corresponding getter method (if any -- may be null)
						this.addOrUpdatePropertyDescriptor(pd, propertyName, readMethod,
							writeMethod, indexedReadMethod, indexedWriteMethod);
						continue ALL_METHODS;
					}
					// has a getter corresponding to this setter already been found by the wrapped BeanInfo?
					if (indexedReadMethod != null
						...
						this.addOrUpdatePropertyDescriptor(pd, propertyName, readMethod,
							writeMethod, indexedReadMethod, method);
						continue ALL_METHODS;
					}
				}
			...
			for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
				...
				for (PropertyDescriptor existingPD : this.propertyDescriptors) {
					if (method.equals(pd.getReadMethod())
						...
						if (existingPD.getReadMethod() == null) {
							// no -> add it now
							this.addOrUpdatePropertyDescriptor(pd, pd.getName(), method,
								pd.getWriteMethod());
						}
				...
				if (method == pd.getReadMethod()
					|| (pd instanceof IndexedPropertyDescriptor && method == ((IndexedPropertyDescriptor) pd)
						.getIndexedReadMethod())) {
					// yes -> copy it, including corresponding setter method (if any -- may be null)
					if (pd instanceof IndexedPropertyDescriptor) {
						this.addOrUpdatePropertyDescriptor(pd, pd.getName(), pd.getReadMethod(),
							pd.getWriteMethod(),
	...
	private void addOrUpdatePropertyDescriptor(PropertyDescriptor pd, String propertyName,
			...
			if (existingPD.getName().equals(propertyName)) {
				...
				if (existingPD.getReadMethod() != null) {
					if (readMethod != null
						...
						|| writeMethod != null
						&& existingPD.getReadMethod().getReturnType() != writeMethod
							.getParameterTypes()[0]) {
				...
				if (existingPD.getWriteMethod() != null) {
					if (readMethod != null
						&& existingPD.getWriteMethod().getParameterTypes()[0] != readMethod
							.getReturnType()
						|| writeMethod != null
						&& existingPD.getWriteMethod().getParameterTypes()[0] != writeMethod
							.getParameterTypes()[0]) {
						// no -> add a new descriptor for it below
						break;
					}
				...
				if (existingPD instanceof IndexedPropertyDescriptor) {
					...
					if (existingIPD.getIndexedReadMethod() != null) {
						if (indexedReadMethod != null
							&& existingIPD.getIndexedReadMethod().getReturnType() != indexedReadMethod
								.getReturnType()
							|| indexedWriteMethod != null
							&& existingIPD.getIndexedReadMethod().getReturnType() != indexedWriteMethod
								.getParameterTypes()[1]) {
							// no -> add a new descriptor for it below
							break;
						}
					...
					if (existingIPD.getIndexedWriteMethod() != null) {
						if (indexedReadMethod != null
							&& existingIPD.getIndexedWriteMethod().getParameterTypes()[1] != indexedReadMethod
								.getReturnType()
							|| indexedWriteMethod != null
							&& existingIPD.getIndexedWriteMethod().getParameterTypes()[1] != indexedWriteMethod
								.getParameterTypes()[1]) {
							// no -> add a new descriptor for it below
							break;
						}
		...
		if (pd == null) {
			try {
				if (indexedReadMethod == null && indexedWriteMethod == null) {
					...
					pd = new IndexedPropertyDescriptor(propertyName, readMethod, writeMethod,
				...
				logger
					.warn(format(
						"Could not create new PropertyDescriptor for readMethod [%s] writeMethod [%s] "
								+ "indexedReadMethod [%s] indexedWriteMethod [%s] for property [%s]. Reason: %s",
						readMethod, writeMethod, indexedReadMethod, indexedWriteMethod,
	...
	public PropertyDescriptor[] getPropertyDescriptors() {
		return this.propertyDescriptors.toArray(new PropertyDescriptor[this.propertyDescriptors
			.size()]);
	}
	
	/**
	 * Sorts PropertyDescriptor instances alphanumerically to emulate the
