...
public final class CachedIntrospectionResults {
	 ...
	 * cache-safe.
	 */
	static final Set<ClassLoader> acceptedClassLoaders = new ConcurrentHashSet<ClassLoader>();
	 ...
	 */
	static final Map<Class, Object> classCache = new ConcurrentHashMap<Class, Object>();
	 ...
	 * ClassLoader.
	 * <p>
	 * Any <code>acceptClassLoader</code> call at application startup should be
	 ...
	 * deregistering the ClassLoader (and any of its children) from the
	 * acceptance list.
	 ...
	 * so we can live with doing the occasional unnecessary lookup at startup
	 * only.
	...
	static CachedIntrospectionResults forClass(Class beanClass) throws BeansException {
		...
		if (results == null) {
			...
			if (fullyCacheable
				|| !ClassUtils.isPresent(beanClass.getName() + "BeanInfo",
	...
	private static boolean isClassLoaderAccepted(ClassLoader classLoader) {
		...
		ClassLoader[] acceptedLoaderArray = acceptedClassLoaders
			.toArray(new ClassLoader[acceptedClassLoaders.size()]);
	 ...
	 * @throws BeansException in case of introspection failure
	 */
	private CachedIntrospectionResults(Class beanClass, boolean cacheFullMetadata)
		...
		try {
			...
			for (PropertyDescriptor pd : pds) {
				...
				if (logger.isTraceEnabled()) {
					logger
						.trace("Found bean property '"
								+ pd.getName()
								+ "'"
								+ (pd.getPropertyType() != null ? " of type ["
																	+ pd.getPropertyType()
																		.getName() + "]" : "")
			...
			throw new FatalBeanException("Failed to obtain BeanInfo for class ["
	...
	private PropertyDescriptor buildGenericTypeAwarePropertyDescriptor(Class beanClass,
																		PropertyDescriptor pd) {
		try {
			return new GenericTypeAwarePropertyDescriptor(beanClass, pd.getName(),
				pd.getReadMethod(), pd.getWriteMethod(), pd.getPropertyEditorClass());
		} catch (IntrospectionException ex) {
			throw new FatalBeanException("Failed to re-introspect class [" + beanClass.getName()
											+ "]", ex);
		}
	}
