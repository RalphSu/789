...
public class CommonUtil {
	...
	public static boolean validationImage(String image) {
		String[] imageFormat = { "bmp", "gif", "jpg", "jpeg", "png" };
		boolean flag = false;
	...
	/***
	 * 获取项目缩略图（若项目缩略图存在则显示缩略图，若缩略图不存在则显示默认缩略图）
	 * @param url
	 * @param request
	 * @return
	 */
	public static String getProjectThumbnailUrl(String url,HttpServletRequest request){
		if(url != null && !"".equals(url.trim())) {
			int i = url.lastIndexOf("/uploadfile/images/");
			if(i != -1){
				url = url.substring(i);
			}
		...
		return AppConstantsUtil.getProjectDefaultThumbnailUrl();
	}
