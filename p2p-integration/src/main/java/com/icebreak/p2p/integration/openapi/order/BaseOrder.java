...
public abstract class BaseOrder {
	...
	private String partnerId = Constants.PARTNER_ID;
	 ...
	 * //TODO 需要支持分组验证，此类需求见RealNameCertSaveRequest
	 */
	public void validate() {

		Validator validator = YjfOpenApiValidatorFactory.INSTANCE
				.getValidator();
		Set<ConstraintViolation<BaseOrder>> validateResultSet = validator
				.validate(this);
	...
	private String concatMessage(
			Set<ConstraintViolation<BaseOrder>> validateResultSet) {
		StringBuilder validateResultMsgBuilder = new StringBuilder();
		for (ConstraintViolation<BaseOrder> constraintViolation : validateResultSet) {
			validateResultMsgBuilder
		...
		if (validateResultMsgBuilder.length() > 1) {
			...
			return validateResultMsgBuilder.length() > 1 ? StringUtils
					.removeEnd(validateResultMsgBuilder.toString(), ",")
					: validateResultMsgBuilder.toString();
	}
